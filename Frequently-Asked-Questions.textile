...
Capistrano requires Ruby 1.8.7 (MRI) with RubyGems installed. Some people have had limited success with JRuby. Some versions of Ruby 1.8.7 had a "thread selecting bug":http://redmine.ruby-lang.org/issues/show/1993, if you are affected you should upgrade immediately. A workaround is included in Net-SSH > 2.0.14.
...
"Capfile" is to Capistrano as "Rakefile" is to Rake, and "Makefile" is to make. It is the file that cap (Capistrano's command-line client) will try to find and load by default. This is the file where your deployment environment is bootstrapped.
...
h4. What is the :password variable?
...
h3.  What is the current_path variable 

h3.  What is the release_path variable 

h3.  What is the version variable 
...
h3. How do I make a custom role?
...
h3. How do I get a terse list of command line options?

At the command prompt, type "cap -h"

h3. How do I get a verbose list of command line options?

At the command prompt, type "cap -H"

h3. How do I get a list of available tasks?
...
h3. How do I get a full list of tasks including internal or undocumented tasks?

At the command prompt, in the root of your project directory (or wherever your Capfile is), type "cap -Tv"

h3. How do I learn more about a specific task?
...
h3. How do I use a Capfile at a different location than my current directory?

If you want to specify the location to the capfile, use the -f flag. For example: "cap -f /home/demo/SpecialCapFile deploy"

h3. How do I tell what version of Capistrano I'm running?

At the command prompt, type "cap -V".

h2. Standard tasks

h3. How do I verify the dependencies on the server?
...
h3. How do I define my own dependencies?
...
Depend takes a "location" argument (:remote or :local), a "type" argument - (:gem, :command, directory), and one or more additional arguments depending on the context. 

  [TODO - Looked up depend(location, type, args) doc - what are all of the options for location, type, and arguments? Not documented fully in rdoc.]

h3. How do I setup the default Capistrano directory structure on my server?
...
h3. Is it safe to run deploy:setup multiple times?
...
h3. How do I prepare the database?
...
h3. How does Capistrano know how to "start" my application?
...
h3. How do I write custom tasks?
...
h3. How do I execute a custom task?
...
h3. How do I "hook" in my methods to run before or after standard deployment tasks?
...
h3. What if I'm not using Subversion?
...
h3. What are deployment "strategies"?
...
h3. Why are all app releases on the servers file system?

This allows us to roll back to a previous release if we ever deploy a lemon.

h3. Why does Capistrano put new releases in a separate directory and symlink to them?
...
h3. What is the (deploy_to)/current/REVISION file and why is it useful?
...
h3. How can I tell what changes have been made since my last deploy?
...
h3. What happens when a deploy goes bad? How can I switch back to the previously deployed version?
...
h3. Where is a good place to store passwords?
...
h3. How can I get Capistrano to prompt before executing each command?
...
h3. How can I tunnel SSH connections through a "gateway" server? Why would I want/need to?
